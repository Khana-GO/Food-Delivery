...
export default function MenuItemsScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F7F7F5' }}>
      ...
      <View style={{ backgroundColor: '#B5122A', paddingTop: isVeryCompact ? 28 : 34, paddingBottom: 10, paddingHorizontal: isVeryCompact ? 16 : 20, borderBottomLeftRadius: 32, borderBottomRightRadius: 32 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, gap: 6, minWidth: 0 }}>
            <TouchableOpacity
              ...
              style={{ width: isVeryCompact ? 28 : 32, height: isVeryCompact ? 28 : 32, borderRadius: 14, backgroundColor: 'rgba(255,255,255,0.18)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.25)' }}
            >
              <Feather name="arrow-left" size={isVeryCompact ? 14 : 16} color="#FFFFFF" />
            </TouchableOpacity>
            <View style={{ width: isVeryCompact ? 28 : 32, height: isVeryCompact ? 28 : 32, borderRadius: 9, backgroundColor: 'rgba(255,255,255,0.18)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.25)' }}>
              <Feather name="book-open" size={isVeryCompact ? 12 : 14} color="#FFFFFF" />
            </View>
            <View style={{ flex: 1, minWidth: 0 }}>
              <Text
                style={{ fontSize: isVeryCompact ? 15 : 16, fontWeight: '800', color: '#FFFFFF', letterSpacing: -0.3 }}
                numberOfLines={1}
              >
                Menu Items
              </Text>
              <Text style={{ fontSize: 10, color: 'rgba(255,255,255,0.8)', fontWeight: '600' }} numberOfLines={1}>
          ...
          <TouchableOpacity
            activeOpacity={0.85}
            style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFFFFF', paddingHorizontal: isVeryCompact ? 8 : 12, paddingVertical: isVeryCompact ? 6 : 7, borderRadius: 999, gap: 5, shadowColor: '#7F0D1D', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 8, elevation: 4 }}
            onPress={() =>
              router.push({
          ...
          >
            <Feather name="plus" size={isVeryCompact ? 12 : 14} color="#B5122A" />
            {!isVeryCompact ? <Text style={{ fontSize: 11, fontWeight: '800', color: '#B5122A' }}>Add Item</Text> : null}
          </TouchableOpacity>
        </View>
      </View>
            ...
            {restaurants!.map((r) => {
              ...
              return (
                <TouchableOpacity
                  key={r.id}
                  activeOpacity={0.85}
                  className={`flex-row items-center px-3.5 py-2 border rounded-full shadow-sm ${
                    isActive
                ...
                >
                  <View className={`h-5 w-5 items-center justify-center rounded-full mr-2 ${isActive ? 'bg-white/20' : 'bg-gray-100'}`}>
                    <Feather
                      name="shopping-bag"
                      size={11}
      ...
      {/* Search — responsive: prevents text cut on 320-360px */}
        <View className="px-4 pt-2.5 pb-2 bg-gray-50">
          <View
            className={`flex-row items-center bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden ${
              isVeryCompact ? 'h-10 px-3' : 'h-11 px-3'
            }`}
          >
            <View className={`${isVeryCompact ? 'h-6 w-6' : 'h-7 w-7'} items-center justify-center rounded-full bg-[#FEF2F2] shrink-0`}>
              <Feather name="search" size={isVeryCompact ? 13 : 14} color="#B91C1C" />
            </View>
            <TextInput selectionColor="rgba(15,23,42,0.16)" cursorColor="#334155"
              className={`${isVeryCompact ? 'text-[12px]' : 'text-[14px]'} font-normal text-gray-900`}
              placeholder={isVeryCompact ? 'Search dishes...' : isCompact ? 'Search menu...' : 'Search menu items, categories...'}
              placeholderTextColor="#94A3B8"
              style={{ flex: 1, minWidth: 0, marginLeft: isVeryCompact ? 8 : 10, fontWeight: '400', paddingVertical: 0 } as any}
              value={searchInput}
            ...
            {searchInput.length > 0 ? (
              ...
              >
                <Feather name="x-circle" size={isVeryCompact ? 15 : 16} color="#94A3B8" />
              </TouchableOpacity>
            ) : (
              <View className={`ml-2 rounded-full bg-gray-50 border border-gray-100 shrink-0 ${isVeryCompact ? 'px-2 py-0.5' : 'px-2 py-0.5'}`}>
                <Text className={`${isVeryCompact ? 'text-[10px]' : 'text-[10px]'} font-bold text-gray-500`} numberOfLines={1} allowFontScaling={false}>
                  {totalCount} items
                </Text>
              </View>
            )}
            ...
            return (
              <TouchableOpacity
                ...
                className={`flex-1 min-w-0 flex-row items-center border rounded-2xl overflow-hidden ${
                  isVeryCompact ? 'gap-1.5 p-2' : isCompact ? 'gap-2 p-2' : 'gap-2 p-2.5'
                } ${isActive ? stat.activeClass : stat.inactiveClass}`}
                onPress={() => setAvailability(stat.key)}
              >
                <View
                  className={`items-center justify-center rounded-xl shrink-0 ${isVeryCompact ? 'h-7 w-7' : 'h-8 w-8'} ${
                    isActive ? 'bg-white/20' : stat.key === 'available' ? 'bg-[#DCFCE7]' : stat.key === 'unavailable' ? 'bg-[#FEE2E2]' : 'bg-gray-100'
                  }`}
                >
                  <Feather
                    name={stat.icon}
                    size={isVeryCompact ? 13 : 14}
                ...
                <View className="flex-1 min-w-0">
                  <Text
                    ...
                    className={`font-extrabold leading-none ${isVeryCompact ? 'text-sm' : 'text-[15px]'} ${isActive ? stat.textClass : 'text-gray-900'}`}
                  >
                    {stat.value}
                  </Text>
                  <Text
                    ...
                    adjustsFontSizeToFit
                    allowFontScaling={false}
                    className={`font-semibold ${isVeryCompact ? 'text-[8px]' : 'text-[9px]'} ${isActive ? 'text-white/80' : 'text-gray-500'}`}
                  >
                    {stat.label}
                  </Text>
                </View>
        ...
        {categories && categories.length > 0 && (
          <View className="bg-white border-y border-gray-100">
            ...
            >
              <TouchableOpacity
                activeOpacity={0.85}
                className={`px-3.5 py-2 rounded-full border shadow-sm ${
                  selectedCategory === ''
                    ? 'bg-[#B91C1C] border-[#7F1D1D] shadow-md'
                    : 'bg-white border-gray-200'
                }`}
              ...
              {categories.map((cat) => {
                ...
                return (
                  <TouchableOpacity
                    key={cat.id}
                    activeOpacity={0.85}
                    className={`px-3.5 py-2 border rounded-full shadow-sm flex-row items-center gap-1.5 ${
