...
import { buildStatementPdf, triggerDownload } from '@/lib/statement-pdf';
...
export default function EarningsScreen() {
  ...
  const handleDownloadStatement = useCallback(async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        const { bytes, fileName } = buildStatementPdf({
          headerTitle: 'KhanaGo — Earnings Statement',
          headerSub: `LAST 30 DAYS • ${new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })}`,
          summary: [
            { label: 'Total (30 days)', value: rs(computed.month) },
            { label: 'Delivered orders', value: String(computed.thisMonthTx.length) },
          ],
          rows: computed.thisMonthTx.map((o: any) => ({
            id: `#${String(o.id).slice(0, 8).toUpperCase()}`,
            customer: o.customerName || o.customer || 'Customer',
            date: orderDate(o).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }),
            amount: rs(parseFloat(o.totalAmount) || 0),
          })),
          totalLabel: 'Total Earnings (30 days)',
          totalValue: rs(computed.month),
          footer: `Generated by KhanaGo Restaurant • ${new Date().toISOString().slice(0, 10)} • All amounts in NPR`,
        });
        triggerDownload(bytes, fileName);
        Alert.alert('Statement downloaded', `${fileName} saved to your downloads.`);
    ...
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      ...
      <View style={{ backgroundColor: Colors.primary, paddingTop: insets.top + 12, paddingBottom: 16, paddingHorizontal: 20, borderBottomLeftRadius: Radius['3xl'], borderBottomRightRadius: Radius['3xl'] }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          <TouchableOpacity onPress={() => goBack('/(restaurant-owner)')} style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.18)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.25)' }} activeOpacity={0.7}>
